...
from randomprune import RandomPrune, RandomPruneSampler
...
trainset = torchvision.datasets.CIFAR100(root='/tmp/cifar100', train=True, transform=train_transform, download=True)
trainset = RandomPrune(trainset, prune_ratio=args.ratio, num_epoch=args.num_epoch)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=args.shuffle, sampler = RandomPruneSampler(trainset))

testset = torchvision.datasets.CIFAR100(root='/tmp/cifar100', train=False, download=True, transform=test_transform)
...
fn = 'result/{}{}-{}-epoch{}-batchsize{}-pct{}-labelsm{}-{}_cifar100_RandomPruned_log.json'.format(
                                                                args.model,
